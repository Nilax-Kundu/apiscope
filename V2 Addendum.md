Invariants — V2 Addendum

This addendum defines additional invariants introduced in V2, which extends the system from single-run diagnostics to longitudinal observation.

All V1 invariants remain in force.

If a V2 feature violates either the V1 invariants or the addendum below, it does not ship.
...
V2 may report that behavior changed between runs.
...
Language and structure must reflect this neutrality.

Stability Is Earned, Not Assumed

V2 may classify findings as benign only when supported by sustained evidence.

Benign classification requires repeated observation across runs.
...
Confidence in V2 must account for recency and volatility.
...
V1 reports remain truthful within their original window.
...
V2 invariants are enforced through:
...
Any feature that requires exceptions to these invariants must first amend this document.

Final note

V2 increases context, not confidence.

It is designed to make change visible without turning time into blame, explanation, or authority.
